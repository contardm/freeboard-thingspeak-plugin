...
// ├────────────────────────────────────────────────────────────────────┤ \\
// │ Freeboard widget plugin for ThingSpeak based on Highcharts         │ \\
// └────────────────────────────────────────────────────────────────────┘ \\
(function() {
	...
	var highchartsLineWidgetSettings = [{
		"name": "feeds",
		"display_name": "Nr. of feeds",
		"type": "text",
		"description": "Specify the last number of feeds  you want to see.",
		"default_value": 60
	}, {
		"name": "blocks",
	...
	for (i = 1; i <= MAX_NUM_SERIES; i++) {
		...
		var fieldSource = {
			"name": "fieldname" + i,
			"display_name": "Series " + i + " - Field",
			"type": "text",
			"description":"Field where data is stored (eg:field1, field2, ...)",
		...
		highchartsLineWidgetSettings.push(fieldSource);
	...
	freeboard
		.loadWidgetPlugin(
			{
				"type_name": "thingspeak-timeseries",
				"display_name": "Time series (thingspeak.com)",
				...
				newInstance: function(settings, newInstanceCallback) {
					newInstanceCallback(new thingspeakTimeseriesWidgetPlugin(
						settings));
				}
			}
		);

	var thingspeakTimeseriesWidgetPlugin = function(settings) {

		var self = this;
		var currentSettings = settings;

		var thisWidgetId = "thingspeak-widget-timeseries-" + HIGHCHARTS_ID++;
		var thisWidgetContainer = $('<div class="thingspeak-widget" id="' + thisWidgetId + '"></div>');
		...
		self.onCalculatedValueChanged = function(settingName, newValue) {
			...
			if (series) {
				var feeds = currentSettings.feeds;
				var seriesno = settingName;
				var len = series.data.length;
				var shift = false;				

				for (var i=Math.max(newValue.length-feeds, 0); i < newValue.length; i++) {
					var x = new Date(newValue[i].created_at).getTime();
					var y = eval("newValue[i]." + series.options.field);
					if (isNumber(y)) { //check if it is a real number and not text
						var plotMqtt = [x, Number(y)]; //create the array+ "Y"
